...
static inline void unpack_int4_as_int8x16x2(const uint8_t* ptr, int8x16_t& high_decoded, int8x16_t& low_decoded) {
    ...
    // Packed INT4 uses offset-binary nibbles [0,15] where s = u - 8.
...
// Co-located format: scales and packed data interleaved per group.
// Per n_block (4 columns), per group: [4x fp16 scales (8B) | packed int4 data (group_size*2 B)]
// Group stride = 8 + group_size * 2 (72 bytes for group_size=32)

static void cactus_gemv_int4_colocated(
    const int8_t* A,
    const float A_scale,
    ...
    auto process_blocks = [=](size_t block_start, size_t block_end) {
        for (size_t n_block = block_start; n_block < block_end; ++n_block) {
            ...
            const uint8_t* block_base = B_colocated + n_block * num_groups * group_stride;

            size_t g = 0;
            for (; g + 1 < num_groups; g += 2) {
                const uint8_t* grp0 = block_base + g * group_stride;
                const uint8_t* grp1 = grp0 + group_stride;

                __builtin_prefetch(grp0 + group_stride * 2, 0, 3);
                __builtin_prefetch(grp0 + group_stride * 2 + 64, 0, 3);
                __builtin_prefetch(grp0 + group_stride * 3, 0, 3);

                float16x4_t scales0_f16 = vld1_f16(reinterpret_cast<const __fp16*>(grp0));
                float16x4_t scales1_f16 = vld1_f16(reinterpret_cast<const __fp16*>(grp1));
                float32x4_t scales0 = vcvt_f32_f16(scales0_f16);
                float32x4_t scales1 = vcvt_f32_f16(scales1_f16);

                const uint8_t* b_data0 = grp0 + 8;
                const uint8_t* b_data1 = grp1 + 8;

                const int8_t* a_ptr0 = A + g * group_size;
                const int8_t* a_ptr1 = A + (g + 1) * group_size;
...

                {
                    int8x16_t a_vec = vld1q_s8(a_ptr0);
                    INT4_BVEC_TYPE b0, b1, b2, b3;
                    INT4_UNPACK(b_data0, b1, b0);

                    acc0 = INT4_DOTQ_LANE(acc0, b0, a_vec, 0);
                    INT4_UNPACK(b_data0 + 16, b3, b2);
                    acc0 = INT4_DOTQ_LANE(acc0, b1, a_vec, 1);
                    acc0 = INT4_DOTQ_LANE(acc0, b2, a_vec, 2);
                    acc0 = INT4_DOTQ_LANE(acc0, b3, a_vec, 3);

                    a_vec = vld1q_s8(a_ptr0 + 16);
                    INT4_UNPACK(b_data0 + 32, b1, b0);

                    acc0 = INT4_DOTQ_LANE(acc0, b0, a_vec, 0);
                    INT4_UNPACK(b_data0 + 48, b3, b2);
                    acc0 = INT4_DOTQ_LANE(acc0, b1, a_vec, 1);
                    acc0 = INT4_DOTQ_LANE(acc0, b2, a_vec, 2);
                    acc0 = INT4_DOTQ_LANE(acc0, b3, a_vec, 3);
                }

                {
                    int8x16_t a_vec = vld1q_s8(a_ptr1);
                    INT4_BVEC_TYPE b0, b1, b2, b3;
                    INT4_UNPACK(b_data1, b1, b0);

                    acc1 = INT4_DOTQ_LANE(acc1, b0, a_vec, 0);
                    INT4_UNPACK(b_data1 + 16, b3, b2);
                    acc1 = INT4_DOTQ_LANE(acc1, b1, a_vec, 1);
                    acc1 = INT4_DOTQ_LANE(acc1, b2, a_vec, 2);
                    acc1 = INT4_DOTQ_LANE(acc1, b3, a_vec, 3);

                    a_vec = vld1q_s8(a_ptr1 + 16);
                    INT4_UNPACK(b_data1 + 32, b1, b0);

                    acc1 = INT4_DOTQ_LANE(acc1, b0, a_vec, 0);
                    INT4_UNPACK(b_data1 + 48, b3, b2);
                    acc1 = INT4_DOTQ_LANE(acc1, b1, a_vec, 1);
                    acc1 = INT4_DOTQ_LANE(acc1, b2, a_vec, 2);
                    acc1 = INT4_DOTQ_LANE(acc1, b3, a_vec, 3);
                }
            ...
            for (; g < num_groups; g++) {
                const uint8_t* grp = block_base + g * group_stride;
                const int8_t* a_ptr = A + g * group_size;

                float16x4_t scales_f16 = vld1_f16(reinterpret_cast<const __fp16*>(grp));
                float32x4_t scales = vcvt_f32_f16(scales_f16);
                const uint8_t* b_data = grp + 8;

                int32x4_t acc = vdupq_n_s32(0);

                int8x16_t a_vec = vld1q_s8(a_ptr);
                INT4_BVEC_TYPE b0, b1, b2, b3;
                INT4_UNPACK(b_data, b1, b0);
                INT4_UNPACK(b_data + 16, b3, b2);
                ...
                a_vec = vld1q_s8(a_ptr + 16);
                INT4_UNPACK(b_data + 32, b1, b0);
                INT4_UNPACK(b_data + 48, b3, b2);
...
static void cactus_gemm_int4_colocated(
    const int8_t* A,
    const float* A_scales,
    const uint8_t* B_colocated,
    __fp16* C,
        ...
        [=](size_t tile_start, size_t tile_end) {
            for (size_t tile_idx = tile_start; tile_idx < tile_end; ++tile_idx) {
                ...
                for (size_t g = 0; g < num_groups; g++) {
                    ...
                    __builtin_prefetch(grp + group_stride * 2, 0, 3);
                    ...
                    INT4_UNPACK(b_data, b01, b00);
                    INT4_UNPACK(b_data + 16, b03, b02);
                    INT4_UNPACK(b_data + 32, b11, b10);
                    INT4_UNPACK(b_data + 48, b13, b12);

                    for (size_t mi = 0; mi < actual_m; mi++) {
                        const int8_t* a_ptr = A + (m_start + mi) * K + g * group_size;
...
void cactus_matmul_int4_colocated(const int8_t* A, const float* A_scales,
    ...
    if (M == 1) {
        cactus_gemv_int4_colocated(A, A_scales[0], B_colocated, C, K, N, group_size);
    } else {
        cactus_gemm_int4_colocated(A, A_scales, B_colocated, C, M, K, N, group_size);
    }
}
