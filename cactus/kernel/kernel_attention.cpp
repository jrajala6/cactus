...
static inline void
cactus_attention_f16_h64(const __fp16 *queries, const __fp16 *keys,
                         const __fp16 *values, __fp16 *output,
                         size_t batch_size, size_t seq_len, size_t kv_seq_len,
                         size_t num_q_heads, size_t num_kv_heads, float scale,
                         size_t position_offset, bool is_causal) {
  ...
  CactusThreading::parallel_for(
      ...
      CactusThreading::Thresholds::ATTENTION, [&](size_t start, size_t end) {
        ...
        for (size_t work = start; work < end; ++work) {
          ...
          const __fp16 *q = queries + batch * q_batch_stride +
                            q_pos * q_seq_stride + q_head * HEAD_DIM;
          __fp16 *o = output + batch * o_batch_stride + q_pos * o_seq_stride +
                      q_head * HEAD_DIM;
...
#pragma unroll
          ...
          size_t kv_end =
              is_causal ? std::min(kv_seq_len, abs_q + 1) : kv_seq_len;
          ...
          for (size_t kv0 = 0; kv0 < kv_end; kv0 += BLOCK_SIZE) {
            ...
            for (size_t i = kv0; i < kv1; i++) {
              ...
              const __fp16 *k = keys + batch * kv_batch_stride +
                                i * kv_seq_stride + kv_head * HEAD_DIM;
...
#pragma unroll
            ...
            for (size_t i = 0; i < kv1 - kv0; i++) {
              ...
              const __fp16 *v = values + batch * kv_batch_stride +
                                (kv0 + i) * kv_seq_stride + kv_head * HEAD_DIM;
...
#pragma unroll
              for (int d = 0; d < 8; d++) {
                ...
                acc_lo[d] =
                    vfmaq_f32(acc_lo[d], vcvt_f32_f16(vget_low_f16(vv)), wv);
                acc_hi[d] =
                    vfmaq_f32(acc_hi[d], vcvt_f32_f16(vget_high_f16(vv)), wv);
              }
            }
...
#pragma unroll
          for (int d = 0; d < 8; d++) {
            float16x8_t out =
                vcombine_f16(vcvt_f16_f32(vmulq_f32(acc_lo[d], invv)),
...
void cactus_attention_f16(const __fp16 *queries, const __fp16 *keys,
  ...
  if (head_dim == 64 && mask == nullptr && window_size == 0) {
    cactus_attention_f16_h64(queries, keys, values, output, batch_size, seq_len,
                             kv_seq_len, num_q_heads, num_kv_heads, scale,
                             position_offset, is_causal);
    return;
  }
        ...
        for (size_t work_idx = start_idx; work_idx < end_idx; ++work_idx) {
          ...
          for (size_t kv_block_start = kv_start; kv_block_start < kv_end;
            ...
            const size_t kv_block_end =
                std::min(kv_block_start + BLOCK_SIZE, kv_end);
            ...
            for (size_t kv_idx = 0; kv_idx < block_size; ++kv_idx) {
              const float attn_weight =
                  block_scores[kv_idx] * current_block_scale;
              if (attn_weight == 0.0f)
                continue;
          ...
          if (running_sum > 0.0f) {
            ...
            for (size_t dim = head_dim_aligned; dim < head_dim; ++dim) {
              o_vec[dim] = static_cast<__fp16>(
                  output_accum_tail[dim - head_dim_aligned] * inv_sum);
            }
          } else {
...
void cactus_attention_hybrid_int8_fp16(
  ...
  const size_t num_quant_groups =
      (head_dim + quant_group_size - 1) / quant_group_size;
  ...
  CactusThreading::parallel_for(
      ...
      [=](size_t start_idx, size_t end_idx) {
        ...
        for (size_t work_idx = start_idx; work_idx < end_idx; ++work_idx) {
          ...
          const __fp16 *Q_base = queries + batch_idx * q_batch_stride;
          const int8_t *K_cached_base =
              keys_cached + batch_idx * kv_cached_batch_stride;
          const int8_t *V_cached_base =
              values_cached + batch_idx * kv_cached_batch_stride;
          const __fp16 *K_new_base = keys_new + batch_idx * kv_new_batch_stride;
          const __fp16 *V_new_base =
              values_new + batch_idx * kv_new_batch_stride;
          __fp16 *O_base = output + batch_idx * o_batch_stride;
          ...
          const size_t absolute_q_pos = position_offset + q_pos;
          size_t kv_end =
              is_causal ? std::min(kv_seq_len, absolute_q_pos + 1) : kv_seq_len;

          // Sliding window: skip tokens outside the window
          size_t kv_start = 0;
          if (window_size > 0 && absolute_q_pos > window_size) {
            kv_start = absolute_q_pos - window_size;
          }
          // Align to BLOCK_SIZE boundary for safe block iteration
          size_t kv_block_start0 = (kv_start / BLOCK_SIZE) * BLOCK_SIZE;

          for (size_t kv_block_start = kv_block_start0; kv_block_start < kv_end;
               kv_block_start += BLOCK_SIZE) {
            const size_t kv_block_end =
                std::min(kv_block_start + BLOCK_SIZE, kv_end);
            const size_t block_size = kv_block_end - kv_block_start;
            ...
            for (size_t kv_idx = 0; kv_idx < block_size; ++kv_idx) {
              ...
              if (kv_pos < cache_len) {
                const int8_t *k_vec = K_cached_base + kv_pos * kv_seq_stride +
                                      kv_head_idx * head_dim;
                const float *k_scale_base =
                    k_scales +
                    (kv_pos * num_kv_heads + kv_head_idx) * num_quant_groups;

                for (size_t quant_group = 0; quant_group < num_quant_groups;
                     quant_group++) {
...
#pragma unroll
                  for (size_t i = 0; i < 4; i++) {
                    ...
                    float32x4_t k_low = vmulq_f32(
                        vcvtq_f32_s32(vmovl_s16(vget_low_s16(k_vec_i16))),
                        k_scale_vec);
                    float32x4_t k_high = vmulq_f32(
                        vcvtq_f32_s32(vmovl_s16(vget_high_s16(k_vec_i16))),
                        k_scale_vec);

                    score_accum_low = vfmaq_f32(score_accum_low, q_low, k_low);
                    score_accum_high =
                        vfmaq_f32(score_accum_high, q_high, k_high);
                  }
                }
              } else {
                const size_t new_pos = kv_pos - cache_len;
                const __fp16 *k_vec = K_new_base + new_pos * kv_seq_stride +
                                      kv_head_idx * head_dim;
            ...
            for (size_t kv_idx = 0; kv_idx < block_size; ++kv_idx) {
              ...
              if (kv_pos < cache_len) {
                const int8_t *v_vec = V_cached_base + kv_pos * kv_seq_stride +
                                      kv_head_idx * head_dim;
...
#pragma unroll
                  for (size_t i = 0; i < 4; i++) {
                    ...
                    float32x4_t v_low = vmulq_f32(
                        vcvtq_f32_s32(vmovl_s16(vget_low_s16(v_vec_i16))),
                        v_scale_vec);
                    float32x4_t v_high = vmulq_f32(
                        vcvtq_f32_s32(vmovl_s16(vget_high_s16(v_vec_i16))),
                        v_scale_vec);
                ...
                const size_t new_pos = kv_pos - cache_len;
                const __fp16 *v_vec = V_new_base + new_pos * kv_seq_stride +
                                      kv_head_idx * head_dim;
...
void cactus_rope_f16(const __fp16 *input, __fp16 *output, size_t batch_size,
  ...
  CactusThreading::parallel_for(
      batch_size * seq_len, CactusThreading::Thresholds::SCALAR_EXPENSIVE,
      [&](size_t start_idx, size_t end_idx) {
...
void cactus_gpt_j_rope_f16(const __fp16 *input, __fp16 *output,
  ...
  CactusRoPEF16::precompute_rope_tables_f16(seq_len + start_pos, rot_dim,
                                            theta);

  const __fp16 *cos_cache =
      CactusRoPEF16::rope_cache_f16.cos_table.data() + start_pos * half_rot_dim;
  const __fp16 *sin_cache =
      CactusRoPEF16::rope_cache_f16.sin_table.data() + start_pos * half_rot_dim;
      ...
      [&](size_t start_idx, size_t end_idx) {
        for (size_t idx = start_idx; idx < end_idx; ++idx) {
          ...
          for (size_t head_idx = 0; head_idx < num_heads; ++head_idx) {
            ...
            constexpr size_t SIMD_WIDTH = 8;
            const size_t vectorized_half_rot_dim =
                (half_rot_dim / SIMD_WIDTH) * SIMD_WIDTH;
            ...
            for (size_t i = vectorized_half_rot_dim; i < half_rot_dim; ++i) {
              ...
              output_ptr[2 * i] =
                  x_first_half * cos_val - x_second_half * sin_val;
            ...
            const size_t copy_end_vec =
                (head_dim / TAIL_SIMD_WIDTH) * TAIL_SIMD_WIDTH;

            for (; copy_idx + TAIL_SIMD_WIDTH <= copy_end_vec;
                 copy_idx += TAIL_SIMD_WIDTH) {
